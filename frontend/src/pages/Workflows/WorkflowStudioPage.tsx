...
export function WorkflowStudioPage() {
  ...
  const handleRestoreSnapshot = (snapshotId: string) => {
    ...
    if (!restored) {
      message.error('Não foi possível restaurar o snapshot.')
      return
    }
    ...
    if (!restoredWorkflow) {
      message.error('Workflow não encontrado após restauração.')
      return
    }
                  ...
                  <div>
                    ...
                    <Space size={6} style={{ marginTop: 6 }}>
                      ...
                      {currentElementConfig ? (
                        <Badge
                          status="success"
                          text={<span style={{ color: '#86efac', fontSize: 11 }}>Configurado</span>}
                        />
                      ) : (
                        <Badge
                          status="warning"
                          text={<span style={{ color: '#fbbf24', fontSize: 11 }}>Sem configuração</span>}
                        />
                      )}
                    </Space>
                  </div>
