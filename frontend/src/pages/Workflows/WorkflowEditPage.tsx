...
import { getWorkflow, updateWorkflow, type WorkflowPayload } from '../../api/workflows'
...
function mapWorkflowToEditableSteps(workflow: Workflow): EditableWorkflowStep[] {
    return [...(workflow.steps ?? [])]
        ...
        .map((step) => ({
            ...
            transitions: (step.transitions ?? []).map((tr) => ({
                toStepOrderIndex: tr.toStepOrderIndex ?? 1,
                triggerAction: tr.triggerAction ?? '',
            })),
            responsibles: (step.responsibles ?? []).map((r) =>
                typeof r === 'string' ? r : r.name
            ),
            receivesNotification: Boolean(step.receivesNotification),
            requiredNotification: Boolean(step.requiredNotification),
            metadata: (step.metadata ?? []).map((meta) => ({
                name: meta.name ?? '',
                label: meta.label ?? '',
                type: meta.type ?? 'text',
                required: Boolean(meta.required ?? meta.isRequired),
...
export function WorkflowEditPage() {
    ...
    useEffect(() => {
        ...
        setSteps(mapWorkflowToEditableSteps(workflow))
    }, [workflow, form])

    const mutation = useMutation({
        mutationFn: (payload: WorkflowPayload) => updateWorkflow(id!, payload),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['workflows'] })
            qc.invalidateQueries({ queryKey: ['workflow', id] })
            message.success('Workflow atualizado!')
    ...
    const updateTransition = (
        ...
        const next = [...steps[stepIndex].transitions]
        next[transitionIndex] = {
            ...next[transitionIndex],
            [field]: value,
        }
        updateStep(stepIndex, 'transitions', next)
    }
    ...
    const updateMetadata = (
        ...
        const next = [...steps[stepIndex].metadata]
        next[metadataIndex] = {
            ...next[metadataIndex],
            [field]: value,
        }
        updateStep(stepIndex, 'metadata', next)
    }
        ...
        <div style={{ padding: 24, background: '#f5f7fb', minHeight: '100vh' }}>
            <Space style={{ marginBottom: 20 }}>
                ...
                <div>
                    ...
                    <Text type="secondary">
                        Atualize etapas, responsáveis, notificações e metadados
                    </Text>
                </div>
            </Space>
            ...
            <Form form={form} layout="vertical" onFinish={handleSave}>
                ...
                <Space>
                    ...
                    <Button onClick={() => navigate(`/workflows/${id}`)}>Cancelar</Button>
                </Space>
            </Form>
        </div>
    )
