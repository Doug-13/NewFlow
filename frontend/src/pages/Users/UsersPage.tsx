        ...
        <div className="positions-tab__selects">
          {[
            ...
            <div key={label} className="positions-tab__select-item">
              <label>{label}</label>
              <Select placeholder="Selecione..." value={value} onChange={onCh}
                options={options} allowClear style={{ width: '100%' }} />
            </div>
          ))}
        </div>
        <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd} disabled={!canAdd}
          className="positions-tab__add-btn">
          Adicionar posição
        </Button>
      </div>
  ...
  const columns = useMemo(() => [
    {
      ...
      render: (_: unknown, r: ExtendedUserListItem) => (
        <div className="users-page__user-cell">
          <Avatar size={42} src={r.photoUrl} icon={!r.photoUrl ? <UserOutlined /> : undefined}
