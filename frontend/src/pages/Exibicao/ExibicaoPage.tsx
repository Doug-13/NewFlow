...
function ColunasTab({
  ...
  // Enriquecer metadataSetName fazendo join com os sets
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {/* Lista de colunas selecionadas (draggable) */}
      {colunas.length > 0 && (
        <div>
      ...
      {/* Seletor inline agrupado por conjunto */}
      {showSelector && (
        <div style={{ border: '1px solid #f0f0f0', borderRadius: 8, overflow: 'hidden' }}>
          {metadataList.length === 0 ? (
            <div style={{ padding: 16, textAlign: 'center' }}>
              <Text type="secondary">Nenhum metadado encontrado.</Text>
            </div>
          ) : (
            <Collapse
              size="small"
...
export function ExibicaoPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['visualizacoes'] })
      message.success('Visualização criada com sucesso.')
      handleClose()
    },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Omit<Visualizacao, 'id'> }) =>
      updateVisualizacao(id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['visualizacoes'] })
      message.success('Visualização atualizada com sucesso.')
      handleClose()
    },
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['visualizacoes'] })
      message.success('Visualização removida.')
    },
  })
  ...
  const handleOpenEdit = (item: Visualizacao) => {
    setEditingItem(item)
    setProcessosVinculados(item.processosVinculados ?? [])
    setColunas(item.colunas ?? [])
  ...
  const transferDataSource = activeProcesses.map((p) => ({
    key: p.id,
    title: p.name,
    description: p.code,
  }))
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: Visualizacao) => {
        ...
        if (cols.length === 0) return <Text type="secondary">—</Text>
        return (
          <Space size={4} wrap>
            {cols.map((c) => <Tag key={c.metadataId}>{c.label}</Tag>)}
          </Space>
        )
      },
    },
    {
      ...
      render: (_: unknown, record: Visualizacao) => {
        ...
        if (vinculados.length === 0) return <Text type="secondary">—</Text>
        return (
          <Space size={4} wrap>
            {vinculados.map((pid) => {
              const proc = processos.find((p) => p.id === pid)
              return proc ? <Tag key={pid}>{proc.name}</Tag> : null
            })}
          </Space>
        )
      },
  ...
  const tabItems = [
      ...
      children: (
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          <Form.Item
            label="Nome"
            name="nome"
            rules={[{ required: true, message: 'Informe o nome da visualização.' }]}
          >
            <Input placeholder="Ex: Minhas Pendências" />
          </Form.Item>

          <Form.Item name="apenasResponsavel" valuePropName="checked" style={{ marginBottom: 4 }}>
            <Checkbox>Apenas itens sob responsabilidade do usuário logado</Checkbox>
          </Form.Item>

          {apenasResponsavel && (
            <Form.Item name="exibirPendenciasAmbientes" valuePropName="checked" style={{ marginBottom: 4, marginLeft: 24 }}>
              <Checkbox>Exibir pendências de todos os ambientes</Checkbox>
            </Form.Item>
          )}

          <Form.Item name="exibirRevisoesAnteriores" valuePropName="checked" style={{ marginBottom: 4 }}>
            <Checkbox>Exibir revisões anteriores</Checkbox>
          </Form.Item>
          <Form.Item name="mostrarPendenciasTreinamento" valuePropName="checked" style={{ marginBottom: 4 }}>
            <Checkbox>Mostrar somente "Pendências de Treinamentos"</Checkbox>
          </Form.Item>
          <Form.Item name="mostrarPendenciasDistribuicao" valuePropName="checked" style={{ marginBottom: 4 }}>
            <Checkbox>Mostrar somente "Pendências de Distribuição"</Checkbox>
          </Form.Item>
          <Form.Item name="mostrarDocumentosCompartilhados" valuePropName="checked" style={{ marginBottom: 4 }}>
            <Checkbox>Mostrar somente "Documentos Compartilhados"</Checkbox>
          </Form.Item>
          <Form.Item name="mostrarItensSeguidos" valuePropName="checked" style={{ marginBottom: 4 }}>
            <Checkbox>Mostrar somente "Itens Seguidos"</Checkbox>
          </Form.Item>
          <Form.Item name="exibirAgrupamentosVazios" valuePropName="checked" style={{ marginBottom: 4 }}>
            <Checkbox>Exibir agrupamentos vazios</Checkbox>
          </Form.Item>
          <Form.Item name="exibirProgressoDatabook" valuePropName="checked" style={{ marginBottom: 4 }}>
            <Checkbox>Exibir progresso Databook</Checkbox>
          </Form.Item>
          <Form.Item name="permiteRolagemHorizontal" valuePropName="checked" style={{ marginBottom: 0 }}>
            <Checkbox>Permite rolagem horizontal</Checkbox>
          </Form.Item>
        </Form>
      ),
    },
    {
      key: 'colunas',
      label: 'Colunas',
      children: <ColunasTab colunas={colunas} onChange={setColunas} />,
    },
    {
      key: 'processos',
      label: 'Processos',
      children: (
        <div>
          <Text type="secondary" style={{ display: 'block', marginBottom: 12 }}>
            Selecione os processos que deseja vincular a esta visualização.
          </Text>
          <Transfer
            ...
            locale={{
              itemUnit: 'processo',
              itemsUnit: 'processos',
              notFoundContent: 'Nenhum processo encontrado',
            }}
            showSearch
