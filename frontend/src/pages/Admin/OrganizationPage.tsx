...
function PositionsTab() {
  ...
  const [preview, setPreview]         = useState<OrgPosition[]>([])   // linhas validadas aguardando confirmação
  ...
  // Shared CSV processor — returns { added, skipped, errors, newPositions }
  ...
  const handleCsvUpload: UploadProps['beforeUpload'] = (file) => {
    ...
    reader.onload = (e) => {
      ...
      setPasteText('')
      setPasteOpen(true)   // abre o modal de preview
      if (errors.length) console.warn('Avisos de importação CSV:\n' + errors.join('\n'))
    }
    reader.readAsText(file, 'UTF-8')
    return false
  }

  // Valida o texto e gera o preview — NÃO confirma ainda
  ...
  // Confirma e aplica o preview nas posições reais
  ...
  const columns = [
    {
      ...
      render: (_: any, r: OrgPosition) =>
        r.userName ? <Tag color="cyan" style={{ margin: 0 }}>{r.userName}</Tag> : <span style={{ color: '#bbb' }}>—</span>,
    },
    {
      title: 'Unidade', key: 'unit',
      render: (_: any, r: OrgPosition) =>
        r.unitName ? <Tag color="green" style={{ margin: 0 }}>{r.unitName}</Tag> : <span style={{ color: '#bbb' }}>—</span>,
    },
    {
      title: 'Área', key: 'area',
      render: (_: any, r: OrgPosition) =>
        r.areaName ? <Tag color="orange" style={{ margin: 0 }}>{r.areaName}</Tag> : <span style={{ color: '#bbb' }}>—</span>,
    },
    {
      title: 'Disciplina', key: 'discipline',
      render: (_: any, r: OrgPosition) =>
        r.disciplineName ? <Tag color="purple" style={{ margin: 0 }}>{r.disciplineName}</Tag> : <span style={{ color: '#bbb' }}>—</span>,
    },
    {
      title: 'Função', key: 'role',
      render: (_: any, r: OrgPosition) =>
        r.roleName ? <Tag color="blue" style={{ margin: 0 }}>{r.roleName}</Tag> : <span style={{ color: '#bbb' }}>—</span>,
    },
    {
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {hasFilter && (
          <Button size="small" type="link" onClick={() => {
            ...
            setFArea(undefined); setFDisc(undefined); setFRole(undefined)
          }}>
            Limpar filtros
          </Button>
        )}
              ...
              {pasteErrors.length > 0 && (
                <div style={{ background: '#fff7e6', border: '1px solid #ffd591', borderRadius: 6, padding: '8px 12px', maxHeight: 120, overflowY: 'auto' }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#d48806', marginBottom: 4 }}>
                    {pasteErrors.length} aviso(s) — nomes não encontrados foram pulados:
                  </div>
                  {pasteErrors.map((e, i) => (
                    <div key={i} style={{ fontSize: 11, color: '#875800' }}>{e}</div>
                  ))}
                </div>
              )}
            </>
          ...
          {preview.length > 0 && (
            <>
              <div style={{ fontSize: 12, color: '#666' }}>
                Revise as posições abaixo antes de confirmar. Clique em <strong>Confirmar importação</strong> para adicioná-las.
              </div>

              {pasteErrors.length > 0 && (
                <div style={{ background: '#fff7e6', border: '1px solid #ffd591', borderRadius: 6, padding: '8px 12px', maxHeight: 80, overflowY: 'auto' }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#d48806', marginBottom: 2 }}>
                    {pasteErrors.length} aviso(s) de nomes não encontrados (linhas ignoradas):
                  </div>
                  {pasteErrors.map((e, i) => (
                    <div key={i} style={{ fontSize: 11, color: '#875800' }}>{e}</div>
                  ))}
              ...
              <Table
                ...
                columns={[
                  ...
                  { title: 'Função',     key: 'role', render: (_,r: OrgPosition) => r.roleName     ? <Tag color="blue"   style={{margin:0}}>{r.roleName}</Tag>     : <span style={{color:'#bbb'}}>—</span> },
                  {
                    ...
                    render: (_,r: OrgPosition) => (
                      <Button type="text" danger size="small" icon={<DeleteOutlined />}
                        onClick={() => setPreview(preview.filter((p) => p.id !== r.id))} />
                    ),
                  },
                ]}
              />
