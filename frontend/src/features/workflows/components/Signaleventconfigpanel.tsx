...
export type SignalEventConfig = {
  ...
  /** Ação a disparar no documento relacionado (ex: 'approve', 'submit', ou id de action custom) */
  targetAction: string
...
export function SignalEventConfigPanel({ workflowId, selectedElement, initialConfig, onSave }: Props) {
  ...
  const handleSubmit = (values: FormValues) => {
    const targetProcess = (processes as Process[]).find((p) => p.id === values.targetProcessId)
            ...
            <Select options={[
              {
                ...
                label: (
                  <Space>
                    <Tag color="blue">Pai → Filho</Tag>
                    <span>Este é o pai — dispara ação no documento filho</span>
                  </Space>
                ),
              },
              {
                ...
                label: (
                  <Space>
                    <Tag color="purple">Filho → Pai</Tag>
                    <span>Este é o filho — dispara ação no documento pai</span>
                  </Space>
                ),
              },
            ]} />
